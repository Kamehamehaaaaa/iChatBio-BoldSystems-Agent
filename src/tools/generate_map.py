...
async def generate_map(state: BoldAgentState):
    ...
    await process.log("Generating geographic map for collection records from Bold systems")
    ...
    if code == 422:
        await process.log(f"generate map failed", data=response_json)
