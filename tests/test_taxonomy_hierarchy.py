...
async def test_session_inactive_returns_early(base_state):
    ...
    result = await taxonomy_hierarchy(base_state)

    assert result == base_state


# ---------------------------
# 2. Successful taxonomy fetch
# ---------------------------
@pytest.mark.asyncio
async def test_successful_taxonomy_fetch(base_state):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"data": "taxonomy"}

    with patch("requests.get", return_value=mock_response):
        result = await taxonomy_hierarchy(base_state)

    assert result == {"taxonomy": True}

    base_state["process"].log.assert_any_await(
        "Fetching taxonomy hierarchy from Bold systems"
    )
...
async def test_422_error_sets_session_inactive(base_state):
    ...
    base_state["process"].log.assert_any_await(
        "fetching taxonomic hierarchy failed",
...
async def test_query_id_is_url_encoded(base_state):
    ...
    with patch("requests.get", return_value=mock_response) as mock_get:
        await taxonomy_hierarchy(base_state)
